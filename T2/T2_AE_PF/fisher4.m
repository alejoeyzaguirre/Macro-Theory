function [Vt,At,Ct,St,Ap,Am,Cm,L_barra, Kt,income,Nt,Lt] = fisher4(T,alpha,beta,sigma,theta,r,A,liq)
% Salario.
...
for t = T:-1:1 % Parto de atrás hacia adelante.
    g=productividad(t);
    b(t) = (b(t+1)-w*g)/(1+r); % Usando recursión hacia atrás vamos llenando
    % Nuestro vector de deuda máxima que puede adquirir en todo t.
    % No multiplicamos por n porque es la deuda max (n=1)!

    % Deuda máxima se dará cuando trabaja n = 1. 
...
for t = T:-1:1 
        ...
        % es la deuda máxima!
        c = (1/(1+theta))*(w*g + (1+r).*A(b_pos:end) - A(b_pos_p:end)');
        n=1-(c*theta)/(w*g); % Nótese como n se calcula a partir de c!
        n(n<0)=NaN;
        n(n>1)=NaN;
        c = w*g*n + (1+r).*A(b_pos:end) - A(b_pos_p:end)';
        % Calculamos policy!
        ...
        Vaux = util4(c,theta,n) + beta*Vt(b_pos_p:end,t+1)'; % Sumo vector (
        ...
        Cm(b_pos:end,t) = w*g.*n(t) + (1+r).*A(b_pos:end) - A(b_pos_p - 1 + P);
...
G=arrayfun(@productividad, 1:T+1);
...
% a_t+1 para todo t. 
Ct = (w*G(1:T) + (1+r)*At(1:T) - At(2:T+1)).*(1/(1+theta)); %Policy function consumption.
Nt=1-(Ct*theta)./(w*G(1:T)); % A partir de Ct calculamos Nt.
Lt=Nt.*G(1:T); % Vemos la trayectoria del trabajo de cada agente.
...
% Buscamos oferta agregada de Trabajo (Depende de cuánto trabajan agentes!):
L_barra= sum((1/T).*G(2:T+1).*Nt(1:T));
