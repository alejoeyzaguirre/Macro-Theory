function [Vt,At,Ct,St,Ap,Am,Cm,L_barra, Kt,income] = fisher(T,alpha,beta,sigma,rho,r,A,Z,liq)
% Salario.
...
for t = T:-1:1 
        ...
        % es la deuda máxima!
        c = w*g + (1+r).*A(b_pos:end) - A(b_pos_p:end)'; % Calculamos policy!
        ...
        Cm(b_pos:end,t) = w*g + (1+r).*A(b_pos:end) - A(b_pos_p - 1 + P);
...
% a_t+1 para todo t. 
Ct = w.*G(1:T) + (1+r)*At(1:T) - At(2:T+1); %Policy function consumption.
...
% Buscamos oferta agregada de Trabajo:
L_barra= sum((1/T)*G(2:T+1));

% Ahora encontramos oferta agregada de Capital:
Kt= (alpha/(r+sigma))^(1/(1-alpha))*L_barra;

% Ingreso será entonces salario de cada agente por su respectivo nivel de
% productividad.
income=w.*G(1:T);
